...
<div class="nav-navicon admin-main admin-sidebar">
    ...
    <div class="sideMenu am-icon-dashboard" style="color:#aeb2b7; margin: 10px 0 0 0;"> 欢迎系统管理员：<?php echo (session('adminname')); ?> <a href="<?php echo U('index/index');?>" style="color: #aeb2b7;">注销</a></div>
    <div class="sideMenu">
...
<div class="admin-biaogelist">
    ...
    <div class="listbiaoti am-cf">
      <ul class="am-icon-flag on">用户信息管理</ul>
      
      <dl class="am-icon-home" style="float: right;"> 当前位置： 用户列表 > <a href="<?php echo U('index/main');?>">首页</a></dl>
    </div>

    <form class="am-form am-g" action="<?php echo U('yonghuliebiao');?>" method="post">
          <table width="100%" class="am-table am-table-bordered am-table-radius am-table-striped">
            <thead>
              <tr class="am-success">
                ...
                <th class="table-author am-hide-sm-only">微博</th>
                <th class="table-date am-hide-sm-only">积分</th>
                <th class="table-date am-hide-sm-only">等级</th>
                <th class="table-date am-hide-sm-only">头像</th>
            ...
            <?php if(is_array($re)): $i = 0; $__LIST__ = $re;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$er): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td><?php echo ($er["phone"]); ?></td>
                <td class="am-hide-sm-only"><?php echo ($er["email"]); ?></td>
                <td class="am-hide-sm-only"><?php echo ($er["qq"]); ?></td>
                <td class="am-hide-sm-only"><?php echo ($er["wechat"]); ?></td>
                <td class="am-hide-sm-only"><?php echo ($er["weibo"]); ?></td>
                <td class="am-hide-sm-only"><?php echo ($er["number"]); ?></td>
                <td class="am-hide-sm-only"><?php echo ($er["grade"]); ?></td>
                <td class="am-hide-sm-only"><img style="width: 40%;height: 40%;" src="/sign/Public/<?php echo ($er["img"]); ?>"></td>
                <td><div class="am-btn-toolbar">
                    <div class="am-btn-group am-btn-group-xs">
                      <a href="<?php echo U('xiugaiyonghu',array('id'=>$er['id']));?>"><button name="<?php echo ($er["id"]); ?>" type="button" title="修改用户" class="am-btn am-btn-default am-btn-xs am-text-success am-round"><span class="am-icon-pencil-square-o"></span> </button></a>
                      <a href="#"><button name="shanchu" type="button" id="<?php echo ($er["id"]); ?>" title="删除用户" class="am-btn am-btn-default am-btn-xs am-text-danger am-round"><span class="am-icon-trash-o"></span></button></a>
                    </div>
                  </div></td>
        ...
        <hr />
          <p>
        备注：操作图标含义
         <a class="am-text-success am-icon-file" title="修改用户"> 修改用户</a> 
