...
<div class="admin-biaogelist">
    ...
    <div class="listbiaoti am-cf">
      <ul class="am-icon-volume-up">售后信息管理</ul>
      
      <dl class="am-icon-home" style="float: right;"> 当前位置： 售后申请列表 > <a href="<?php echo U('index/main');?>">首页</a></dl>
    </div>

    <form class="am-form am-g" action="<?php echo U('shenqingliebiao');?>" method="post">
          <table width="100%" class="am-table am-table-bordered am-table-radius am-table-striped">
            <thead>
              <tr class="am-success">
                <th class="table-title">用户名</th>
                <th class="table-type">订单名称</th>
                <th class="table-date am-hide-sm-only">申请信息</th>
            ...
            <?php if(is_array($re)): $i = 0; $__LIST__ = $re;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$er): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td class="am-hide-sm-only"><?php echo ($er["message"]); ?></td>
                <td class="am-hide-sm-only"><?php echo (date("Y-m-d H:i:s",$er["time"])); ?></td>
                <td><div class="am-btn-toolbar">
                    <div class="am-btn-group am-btn-group-xs">
                      <a href="<?php echo U('xiugaijindu',array('id'=>$er['id']));?>"><button name="<?php echo ($er["id"]); ?>" type="button" title="修改申请进度" class="am-btn am-btn-default am-btn-xs am-text-success am-round"><span class="am-icon-pencil-square-o"></span> </button></a>
                    </div>
                  </div></td>
        ...
        <hr />
          <p>
        备注：操作图标含义
         <a class="am-text-success am-icon-file" title="修改申请进度"> 修改申请进度</a> 
        </p>
        </form>
