...
<div class="admin-biaogelist">
    ...
    <div class="listbiaoti am-cf">
      <ul class="am-icon-users">收货地址管理</ul>
      
      <dl class="am-icon-home" style="float: right;"> 当前位置： 收货地址列表 > <a href="<?php echo U('index/main');?>">首页</a></dl>
      
      <dl>
        <a href="<?php echo U('tianjiashouhuo');?>"><button type="button" class="am-btn am-btn-danger am-round am-btn-xs am-icon-plus">添加收货地址</button></a>
      </dl>     
    </div>

    <form class="am-form am-g" action="<?php echo U('shouhuoliebiao');?>" method="post">
          <table width="100%" class="am-table am-table-bordered am-table-radius am-table-striped">
            ...
            <?php if(is_array($re)): $i = 0; $__LIST__ = $re;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$er): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td><?php echo ($er["uid"]); ?></td>
                <td class="am-hide-sm-only"><?php echo ($er["address"]); ?></td>
                <td><div class="am-btn-toolbar">
                    <div class="am-btn-group am-btn-group-xs">
                      <a href="<?php echo U('xiugaishouhuo',array('id'=>$er['id']));?>"><button name="<?php echo ($er["id"]); ?>" type="button" title="修改收货地址" class="am-btn am-btn-default am-btn-xs am-text-success am-round"><span class="am-icon-pencil-square-o"></span> </button></a>
                      <a href="#"><button name="shanchu" type="button" id="<?php echo ($er["id"]); ?>" title="删除收货地址" class="am-btn am-btn-default am-btn-xs am-text-danger am-round"><span class="am-icon-trash-o"></span></button></a>
                    </div>
                  </div></td>
        ...
        <hr />
          <p>
        备注：操作图标含义
         <a class="am-text-success am-icon-file" title="修改收货地址"> 修改收货地址</a> 
         <a class="am-icon-trash-o am-text-danger" title="删除"> 删除收货地址</a>
        </p>
        </form>
